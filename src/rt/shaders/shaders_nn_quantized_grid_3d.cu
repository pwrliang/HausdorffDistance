...
extern "C" __constant__ hd::details::LaunchParamsNNQuantizedGrid<FLOAT_TYPE, 3>
    params;

extern "C" __global__ void __intersection__nn_quantized_grid_3d() {
  auto point_a_id = optixGetPayload_0();
  auto iter = optixGetPayload_2();
  ...
  if (quantized_dist2 <= radius * radius) {
    ...
    // don't need to calculate the minimum distance until we have 3 iterations
    if (iter < 3) {
...
extern "C" __global__ void __anyhit__nn_quantized_grid_3d() {
  optixTerminateRay();
}

extern "C" __global__ void __raygen__nn_quantized_grid_3d() {
  ...
  for (auto i = optixGetLaunchIndex().x; i < in_queue.size();
    ...
    if (sizeof(FLOAT_TYPE) == sizeof(float)) {
      ...
      optixTrace(params.handle, origin, dir, tmin, tmax, 0,
                 ...
                 point_id_a, hit, iter, cmin2_storage);
      cmin2 = *reinterpret_cast<FLOAT_TYPE*>(&cmin2_storage);
    } else {
      uint2 cmin2_storage;
      hd::pack64(&cmin2, cmin2_storage.x, cmin2_storage.y);
      optixTrace(params.handle, origin, dir, tmin, tmax, 0,
                 ...
                 RAY_TYPE_COUNT,       // SBT stride
                 SURFACE_RAY_TYPE,     // missSBTIndex
                 point_id_a, hit, iter, cmin2_storage.x, cmin2_storage.y);
      hd::unpack64(cmin2_storage.x, cmin2_storage.y, &cmin2);
    }
