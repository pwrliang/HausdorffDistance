...
DEFINE_double(radius_step, 1.1, "Step of radius increase (RT only)");
DEFINE_bool(rebuild_bvh, false, "rebuild BVH (RT only)");
DEFINE_bool(shuffle, false, "Shuffle points (RT only)");
DEFINE_int32(parallelism, 1, "Parallelism for casting rays (RT only)");