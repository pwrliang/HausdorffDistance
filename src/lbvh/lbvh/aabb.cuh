...
template <typename T>
__device__ __host__ inline bool intersects(const aabb<T> &lhs,
...
template <typename T>
__device__ __host__ inline bool contains(const aabb<T> &lhs,
                                         const aabb<T> &rhs) noexcept {
  return rhs.lower.x >= lhs.lower.x && rhs.upper.x <= lhs.upper.x &&
         rhs.lower.y >= lhs.lower.y && rhs.upper.y <= lhs.upper.y &&
         rhs.lower.z >= lhs.lower.z && rhs.upper.z <= lhs.upper.z;
}

__device__ __host__ inline aabb<double>
merge(const aabb<double> &lhs, const aabb<double> &rhs) noexcept {
  aabb<double> merged;
...
__device__ __host__ inline float mindist(const aabb<float> &lhs,
                                         const float4 &rhs) noexcept {
...
__device__ __host__ inline double mindist(const aabb<double> &lhs,
                                          const double4 &rhs) noexcept {
...
__device__ __host__ inline float minmaxdist(const aabb<float> &lhs,
                                            const float4 &rhs) noexcept {
...
__device__ __host__ inline double minmaxdist(const aabb<double> &lhs,
                                             const double4 &rhs) noexcept {
...
template <typename T>
__device__ __host__ inline typename vector_of<T>::type
centroid(const aabb<T> &box) noexcept {
