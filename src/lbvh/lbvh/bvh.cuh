...
template <typename Real, typename Object, bool IsConst> struct basic_device_bvh;
...
find_split(UInt const *node_code, const unsigned int num_leaves,
...
struct default_morton_code_calculator {
  ...
  operator()(const Object &, const aabb<Real> &box) noexcept {
    auto p = centroid(box);
    p.x -= whole.lower.x;
    p.y -= whole.lower.y;
    p.z -= whole.lower.z;
