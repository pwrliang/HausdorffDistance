...
template<typename Real>
struct query_contain
{
  __device__ __host__
  query_contain(const aabb<Real>& tgt): target(tgt) {}

  query_contain()  = default;
  ~query_contain() = default;
  query_contain(const query_contain&) = default;
  query_contain(query_contain&&)      = default;
  query_contain& operator=(const query_contain&) = default;
  query_contain& operator=(query_contain&&)      = default;

  __device__ __host__
      inline bool operator()(const aabb<Real>& box) noexcept
  {
    return contains(target, box);
  }
...
template<typename Real>
__device__ __host__
    query_contain<Real> contains(const aabb<Real>& region) noexcept
{
  return query_contain<Real>(region);
}
