...
function run_mri_datasets() {
  ...
  while ((counter < max)); do
    ...
    if [[ -n "$file1" && -n "$file2" ]]; then
      ...
      if [[ "$file1" != "$file2" ]]; then
        run_hd "$out_prefix" "$file1" "$file2" $type "false" $dims false false
        run_hd "$out_prefix" "$file1" "$file2" $type "false" $dims true true
      else
        ((counter -= 2))
      fi
    else
  ...
  for sub_folder in "$root"/*/; do
    ...
    while ((counter < max)); do
      ...
      if [[ -n "$file1" && -n "$file2" ]]; then
        ...
        #  echo "Skipping, $log exists"
        #else
        run_hd "$out_prefix" "$file1" "$file2" "off" "true" 3 false false
        run_hd "$out_prefix" "$file1" "$file2" "off" "true" 3 true true
...
function run_mri() {
  run_mri_datasets "$DATASET_ROOT/BraTS2020_ValidationData" "image" 3 "hybrid" "gpu"
}

function run_modelnet() {
  run_modelnet_datasets "$DATASET_ROOT/ModelNet40" "hybrid" "gpu"
}
